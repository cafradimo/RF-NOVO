...
def carregar_dados_fiscais():
    """Carrega os dados dos fiscais do arquivo Excel"""
    try:
...
def carregar_planilha_master_drive(service, folder_id):
    ...
    try:
        ...
        if arquivos:
            ...
            # CORREÇÃO: Removido o parâmetro delete_on_close que causava erro
            with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as temp_file:
                caminho_temp = temp_file.name
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        with col2:
            # Usar a função compatível para exibir imagens
            if os.path.exists("10.png"):
                # CORREÇÃO: Passar width como parâmetro para a função compatível
            ...
            if st.button("Entrar", type="primary", use_container_width=True, key="login_button"):
                if matricula_input and senha_input:
                    ...
                    if len(matricula_limpa) >= 3 and len(matricula_limpa) <= 4:
                        ...
                        # Verificar se a matrícula existe no arquivo de fiscais
                        agente_info = None
                        if dados_fiscais:
                        ...
                        if agente_info:
                            ...
                            st.error("Matrícula não encontrada no sistema de fiscais. Verifique a matrícula ou contate o administrador.")
                    else:
    ...
    with st.sidebar:
        ...
        if os.path.exists("10.png"):
            ...
            st.warning("Logo não encontrado - arquivo 10.png")
