...
# ATUALIZADO: Com gestão de pastas local e configuração do usuário
...
def obter_caminho_pasta_usuario(matricula):
    ...
    if matricula in config:
        ...
        if os.path.exists(caminho_configurado):
            ...
            # Se não existe mais, remover da configuração
            del config[matricula]
            salvar_configuracao_pasta_usuario(matricula, "")
    return None

# ========== FUNÇÃO PARA CRIAR/CONFIGURAR PASTA LOCAL (COM LOGS) ==========
def configurar_pasta_local(matricula, mostrar_logs=False):
    ...
    """
    try:
        # Nome da pasta baseado na matrícula
        nome_pasta = f"RF-CREA-RJ{matricula}"
        
        # Primeiro verificar se o usuário já tem configuração salva
        ...
        if caminho_configurado:
            ...
            if os.path.exists(caminho_configurado):
                ...
                # Se a pasta não existe mais, remover da configuração
                if mostrar_logs:
        ...
        # ==============================================
        # CORREÇÃO: Detectar se estamos no Streamlit Cloud
        # ==============================================
        is_streamlit_cloud = os.environ.get("STREAMLIT_CLOUD", False)
        ...
        if is_streamlit_cloud:
            ...
            # No Streamlit Cloud, podemos criar no diretório home do usuário
            home_dir = os.path.expanduser("~")
            
            # Tentar criar na pasta Documents (pode não existir no Linux)
            caminho_pasta = os.path.join(home_dir, "Documents", nome_pasta)
            
            # Se Documents não existir, criar no diretório home
            if not os.path.exists(os.path.join(home_dir, "Documents")):
                caminho_pasta = os.path.join(home_dir, nome_pasta)
            ...
            # Opções de locais para criar a pasta (em ordem de preferência)
            locais_possiveis = [
                ...
                os.path.join(os.getcwd(), nome_pasta),                           # 3. Pasta atual
                os.path.join("relatorios", nome_pasta),                          # 4. Pasta relatorios
            ]
            
            # Tenta criar a pasta em cada local possível
            for caminho_tentativa in locais_possiveis:
                try:
                    ...
                    if not os.path.exists(caminho_tentativa):
                        ...
                        if mostrar_logs:
                            st.success(f"✅ Pasta criada em: {caminho_tentativa}")
                    else:
                        if mostrar_logs:
                            st.info(f"📁 Pasta já existe em: {caminho_tentativa}")
                    ...
                    # Se chegou aqui, a pasta é válida e tem permissão
                    if mostrar_logs:
                        st.success(f"🎯 Pasta configurada em: {caminho_tentativa}")
                    ...
                    if mostrar_logs:
                        st.warning(f"❌ Sem permissão em {caminho_tentativa}: {str(e)}")
                    continue
                except Exception as e:
                    if mostrar_logs:
                        st.warning(f"⚠️ Erro em {caminho_tentativa}: {str(e)}")
                    continue
            
            # Se não conseguiu criar em nenhum local, usa o diretório temporário
            caminho_pasta = os.path.join(tempfile.gettempdir(), nome_pasta)
            if mostrar_logs:
                st.warning(f"⚠️ Usando diretório temporário: {caminho_pasta}")
        
        # ==============================================
        # CRIAÇÃO FINAL DA PASTA
        # ==============================================
        try:
            ...
            # Tentar salvar configuração mesmo no fallback
            try:
                salvar_configuracao_pasta_usuario(matricula, caminho_pasta)
            except:
                pass
    ...
    try:
        ...
        if os.path.exists(caminho_completo):
            ...
            try:
                ...
                pass  # Se falhar o log, não impede a operação principal
        ...
        for arquivo in os.listdir(pasta_local):
            if arquivo.lower().endswith('.pdf'):
                ...
                try:
                    ...
                    # Extrair número do relatório do nome do arquivo
                    numero_relatorio = ""
    ...
    try:
        ...
        if os.path.exists(pasta_local):
            ...
            if sistema == 'Windows':
                ...
                # Para outros sistemas ou se não conseguiu abrir
                return pasta_local  # Retorna o caminho para mostrar ao usuário
...
def mostrar_info_pasta(matricula):
    ...
    try:
        ...
        # Informações do sistema de arquivos
        stat_info = os.statvfs(pasta_local) if hasattr(os, 'statvfs') else None
        espaco_livre = ""
        if stat_info:
            espaco_livre_gb = (stat_info.f_bavail * stat_info.f_frsize) / (1024**3)
            espaco_livre = f"{espaco_livre_gb:.1f} GB"
...
def upload_para_google_drive(caminho_arquivo, nome_arquivo, service, shared_drive_id=None, folder_id=None):
    ...
    try:
        ...
        if folder_id:
            ...
            # Se não tiver pasta específica, busca no root do Shared Drive
            query_parts.append(f"'{shared_drive_id}' in parents")
        
        query = " and ".join(query_parts)
        
        # Parâmetros para busca no Shared Drive
        drive_params = {
            'q': query,
            'spaces': 'drive',
        ...
        # Configurar localização (Shared Drive + Pasta)
        parents = []
        if folder_id:
        ...
        # Configurações para Shared Drive
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        st.caption("Versão 1.0 - Relatório de Fiscalização CREA-RJ")
        return
    ...
    if opcao == "OBRA":
        ...
        with st.expander("📁 Informações do Armazenamento", expanded=False):
            ...
            if info_pasta['existe']:
                ...
                with col1:
                    st.metric("Local", "Configurado", delta="✅")
                with col2:
                ...
                if st.button("📂 Abrir Pasta", key="abrir_pasta_main"):
                    resultado = abrir_pasta_local(st.session_state.matricula)
                    if resultado:
            ...
            if st.button("📄 GERAR RELATÓRIO PDF", 
                ...
                try:
                    ...
                    **📊 Resumo Final:**
                    ...
                    if caminho_salvo:
                        resumo_texto += f"\n- **📁 Armazenamento local:** PDF salvo em pasta pessoal"
                ...
                if resultado is True:
                    ...
                    if relatorios_salvos:
                        ...
                        if len(relatorios_salvos) > 5:
                            st.caption(f"... e mais {len(relatorios_salvos) - 5} relatório(s)")
                    else:
