...
# ATUALIZADO: Contador Persistente usando Google Sheets como backend
# FIX: Contador que não zera após cada acesso - Persistência garantida
...
try:
    ...
    st.warning(f"Google Sheets não disponível: {e}")
...
st.set_page_config(
    ...
    menu_items={
        ...
        'Report a bug': None,
        'About': "Sistema de Fiscalização CREA-RJ v3.0 - Contador Persistente"
    }
)
...
# ========== FUNÇÃO AUXILIAR: OBTER CREDENCIAIS GOOGLE ==========
def obter_credenciais_google():
    ...
    try:
        ...
        if 'GOOGLE_SHEETS_CREDENTIALS' in st.secrets:
            creds_text = st.secrets.GOOGLE_SHEETS_CREDENTIALS
            if isinstance(creds_text, str):
                try:
                    return json.loads(creds_text)
                except:
...
# ========== CLASSE CONTADOR PERSISTENTE (ATUALIZADA) ==========
# Substitua a classe ContadorRelatoriosPersistente por esta versão corrigida:

class ContadorRelatoriosPersistente:
    def __init__(self, arquivo_contador=CONTADOR_FILE):
        """
        Contador persistente que usa Google Sheets como backend principal
        e arquivo JSON local como cache/fallback
        """
        self.arquivo_contador = arquivo_contador
        self.ano_atual = dt.now().strftime("%Y")
        self.sheet_name = "CONTADOR_RELATORIOS"
        
        # Inicializar estruturas
        self.contadores = self.carregar_contadores()
        
    def conectar_google_sheets(self):
        """Conecta ao Google Sheets para armazenamento persistente"""
        ...
        try:
            ...
            if not creds_info:
                st.warning("Credenciais do Google não encontradas")
                return None
            ...
            st.warning(f"Erro ao conectar ao Google Sheets: {e}")
            return None
    
    def carregar_contador_do_sheets(self):
        """Carrega contadores do Google Sheets (fonte principal)"""
        gc = self.conectar_google_sheets()
        if not gc:
            return None
        ...
        try:
            ...
            elif 'GOOGLE_SHEETS_SPREADSHEET_ID' in st.secrets:
                spreadsheet_id = st.secrets.GOOGLE_SHEETS_SPREADSHEET_ID
            
            if not spreadsheet_id:
                st.warning("ID da planilha não configurado")
                return None
            
            spreadsheet = gc.open_by_key(spreadsheet_id)
            
            # Tentar obter a planilha de contadores
            try:
                worksheet = spreadsheet.worksheet(self.sheet_name)
            except gspread.WorksheetNotFound:
                # Criar nova planilha para contadores
                ...
                cabecalhos = [
                    "ANO", "MATRICULA", "ULTIMO_SEQUENCIAL", 
                    "ULTIMA_ATUALIZACAO", "TOTAL_RELATORIOS"
                ]
            ...
            # Converter para estrutura de dicionário
            contadores = {}
            for linha in dados:
                # Garantir que temos as colunas necessárias
                if 'ANO' in linha and 'MATRICULA' in linha and 'ULTIMO_SEQUENCIAL' in linha:
                    chave = f"{linha['ANO']}_{linha['MATRICULA']}"
                    try:
                        contadores[chave] = {
                            'ultimo_sequencial': int(linha['ULTIMO_SEQUENCIAL']),
                            'ultima_atualizacao': linha.get('ULTIMA_ATUALIZACAO', ''),
                            'total_relatorios': int(linha.get('TOTAL_RELATORIOS', 0))
                        }
            ...
            st.warning(f"Erro ao carregar contador do Sheets: {e}")
            return None
    
    def salvar_contador_no_sheets(self, ano, matricula, sequencial):
        """Salva/atualiza contador no Google Sheets"""
        gc = self.conectar_google_sheets()
        if not gc:
            return False
        ...
        try:
            ...
            for i, linha in enumerate(dados[1:], start=2):  # Ignorar cabeçalho
                if len(linha) >= 2 and linha[0] == ano and linha[1] == matricula:
                    linha_encontrada = i
                    break
            
            data_atual = dt.now().strftime("%d/%m/%Y %H:%M:%S")
            
            if linha_encontrada > 0:
                # Atualizar linha existente
                worksheet.update_cell(linha_encontrada, 3, sequencial)  # ULTIMO_SEQUENCIAL
                worksheet.update_cell(linha_encontrada, 4, data_atual)  # ULTIMA_ATUALIZACAO
                
                # Incrementar total
                try:
                    total_atual = int(worksheet.cell(linha_encontrada, 5).value or 0)
                    worksheet.update_cell(linha_encontrada, 5, total_atual + 1)
                except:
                    worksheet.update_cell(linha_encontrada, 5, 1)
            else:
            ...
            st.warning(f"Erro ao salvar contador no Sheets: {e}")
            return False
    
    def carregar_contadores(self):
        """
        Carrega contadores - Prioridade:
        1. Google Sheets (persistente na nuvem)
        2. Arquivo JSON local (cache/fallback)
        3. Criar nova estrutura se não existir
        """
        # Primeiro, tentar carregar do Google Sheets
        contadores_sheets = self.carregar_contador_do_sheets()
        ...
        if contadores_sheets is not None:
            # Salvar cache local
            self.salvar_cache_local(contadores_sheets)
            estrutura['fonte'] = 'google_sheets'
            estrutura['contadores'] = contadores_sheets
            return estrutura
        
        # Se Sheets não disponível, tentar arquivo local
        try:
            if os.path.exists(self.arquivo_contador):
                ...
                if isinstance(dados, dict):
                    # Se for a estrutura antiga (apenas dicionário)
                    if 'contadores' in dados:
                        estrutura['fonte'] = 'arquivo_local'
                        estrutura['ultima_atualizacao'] = dados.get('ultima_atualizacao', dt.now().isoformat())
                        estrutura['contadores'] = dados['contadores']
                    else:
                        # Se for apenas dicionário de contadores
                        estrutura['fonte'] = 'arquivo_local'
                        estrutura['ultima_atualizacao'] = dt.now().isoformat()
                        estrutura['contadores'] = dados
                        # Atualizar arquivo com nova estrutura
                        self.salvar_cache_local(dados)
                else:
                    # Estrutura inválida
                    raise ValueError("Estrutura de arquivo inválida")
            else:
                # Arquivo não existe, criar estrutura padrão
                estrutura['fonte'] = 'nova_estrutura'
    ...
    def salvar_cache_local(self, contadores):
        """Salva cache local dos contadores"""
        try:
            dados = {
                'fonte': 'cache_google_sheets',
                'ultima_atualizacao': dt.now().isoformat(),
            ...
            st.warning(f"Erro ao salvar cache local: {e}")
            return False
    
    def gerar_numero_relatorio(self, matricula):
        """
        Gera número do relatório no formato: ANO(4) + MATRÍCULA(4) + SEQUENCIAL(4)
        Prioriza Google Sheets, com fallback para arquivo local
        """
        # Formatar matrícula
        ...
        # DEBUG: Verificar estrutura do contador
        if not isinstance(self.contadores, dict):
            st.error(f"Erro: self.contadores não é um dicionário. Tipo: {type(self.contadores)}")
            # Recriar estrutura
            self.contadores = self.carregar_contadores()
        
        # Garantir que temos a estrutura correta
        if 'contadores' not in self.contadores:
            st.warning("Estrutura de contadores incorreta, recriando...")
            self.contadores = self.carregar_contadores()
        
        # Obter sequencial atual
        if chave in self.contadores['contadores']:
            try:
                ultimo_sequencial = self.contadores['contadores'][chave]['ultimo_sequencial']
            except (KeyError, TypeError):
                # Se a estrutura estiver incorreta, corrigir
                ultimo_sequencial = 0
                self.contadores['contadores'][chave] = {
                    'ultimo_sequencial': 0,
                    'ultima_atualizacao': dt.now().isoformat(),
                    'total_relatorios': 0
                }
        ...
        self.contadores['contadores'][chave] = {
            'ultimo_sequencial': novo_sequencial,
            'ultima_atualizacao': dt.now().isoformat(),
            'total_relatorios': self.contadores['contadores'].get(chave, {}).get('total_relatorios', 0) + 1
        }
        
        # Tentar salvar no Google Sheets (fonte principal)
        sheets_sucesso = self.salvar_contador_no_sheets(ano, matricula_formatada, novo_sequencial)
        
        if sheets_sucesso:
            self.contadores['fonte'] = 'google_sheets'
        else:
            # Fallback: salvar apenas localmente
            if self.salvar_cache_local(self.contadores['contadores']):
                self.contadores['fonte'] = 'arquivo_local_fallback'
            else:
                self.contadores['fonte'] = 'memoria_apenas'
        
        # Formatar número completo
        sequencial_formatado = str(novo_sequencial).zfill(4)
        ...
        st.session_state.historico_geracao.append({
            ...
            'fonte_contador': self.contadores['fonte']
        })
        
        return numero_completo
    
    def obter_ultimo_sequencial(self, matricula):
        """Obtém o último sequencial usado para uma matrícula"""
        try:
            matricula_formatada = matricula.zfill(4)
            chave = f"{self.ano_atual}_{matricula_formatada}"
            
            if 'contadores' in self.contadores and chave in self.contadores['contadores']:
                return self.contadores['contadores'][chave].get('ultimo_sequencial', 0)
            return 0
        except Exception:
            return 0
    ...
    def mostrar_status_contador(self):
        """Mostra status atual do contador"""
        try:
            status = {
                'fonte': self.contadores.get('fonte', 'desconhecida'),
            ...
            # Adicionar informações específicas para Streamlit Cloud
            if IS_STREAMLIT_CLOUD:
                status['ambiente'] = 'Streamlit Cloud'
                status['persistencia'] = 'Google Sheets + Cache Local'
            else:
                status['ambiente'] = 'Local'
                status['persistencia'] = 'Arquivo Local + Google Sheets'
            
            return status
        except Exception as e:
            return {'erro': f'Não foi possível obter status: {str(e)}'}
...
def main():
    ...
    # ========== INICIALIZAR CONTADOR PERSISTENTE (ATUALIZADO) ==========
    ...
    if IS_STREAMLIT_CLOUD:
        with st.expander("⚠️ INFORMAÇÕES IMPORTANTES - STREAMLIT CLOUD", expanded=True):
            st.success("""
            **🎯 CONTADOR PERSISTENTE ATIVADO!**
            
            📌 **Funcionalidades garantidas:**
            ✅ Contador NUNCA zera após cada acesso
            ✅ Números de relatório ÚNICOS e sequenciais
            ✅ Persistência em Google Sheets (nuvem)
            ...
            💡 **Como funciona:**
            1. Cada relatório gera um número único: ANO(4)+MATRÍCULA(4)+SEQUENCIAL(4)
            2. O sequencial é incrementado e salvo na nuvem
            3. Mesmo após fechar o navegador, o contador mantém o último número
            4. Diferentes agentes têm contadores independentes
            """)
    ...
    if not st.session_state.logged_in:
                ...
                if matricula_input and senha_input:
                    ...
                    if len(matricula_limpa) >= 3 and len(matricula_limpa) <= 4:
                        ...
                        if agente_info:
                            ...
                            st.success(f"🔢 **Número do relatório gerado:** {numero_relatorio}")
                            
                            # Formatar número para exibição
                            ano = numero_relatorio[:4]
                            mat = numero_relatorio[4:8]
                            seq = numero_relatorio[8:]
                            
                            st.info(f"📋 **Formato:** {ano}.{mat}.{seq}")
                            st.info(f"📊 **Fonte do contador:** {status_contador.get('fonte', 'N/A')}")
                            
                            # Mostrar próximo sequencial
                            proximo_sequencial = contador_persistente.obter_proximo_sequencial(matricula_formatada)
                            st.info(f"⏭️ **Próximo sequencial:** {proximo_sequencial}")
        ...
        st.caption("Versão 3.0 - Relatório de Fiscalização CREA-RJ com Contador Persistente")
        return
    ...
    with st.sidebar:
        ...
        if IS_STREAMLIT_CLOUD:
            ...
            with st.expander("⚠️ STREAMLIT CLOUD", expanded=True):
                ...
                **🎯 CONTADOR PERSISTENTE ATIVADO**
                ...
                ✅ **Sequenciais mantidos na nuvem**
                ✅ **Funciona após fechar o navegador**
                
                ⚠️ **Arquivos PDF são temporários**
                💡 **Faça download para salvar permanentemente**
                """)
        ...
        with st.expander("⚙️ Gerenciar Pastas", expanded=False):
            ...
            with st.expander("🔢 INFORMAÇÕES DO CONTADOR", expanded=True):
                st.info("**Sistema de Numeração Persistente**")
                
                if st.session_state.get('numero_relatorio_gerado'):
                    numero = st.session_state.numero_relatorio_gerado
                    ano = numero[:4]
                    mat = numero[4:8]
                    seq = numero[8:]
                    
                    st.metric("Número Atual", f"{ano}.{mat}.{seq}")
                    
                    ultimo_seq = contador_persistente.obter_ultimo_sequencial(st.session_state.matricula)
                    proximo_seq = contador_persistente.obter_proximo_sequencial(st.session_state.matricula)
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        st.metric("Último", ultimo_seq)
                    with col2:
                        st.metric("Próximo", proximo_seq)
                
                # Botão para ver status detalhado
                if st.button("📊 Ver Status do Contador", key="ver_status_contador"):
                    status = contador_persistente.mostrar_status_contador()
                    
                    st.markdown("**Status do Sistema:**")
                    st.json(status)
                    
                    # Mostrar contadores ativos
                    contadores = contador_persistente.contadores.get('contadores', {})
                    if contadores:
                        st.markdown("**Contadores Ativos:**")
                        for chave, valor in list(contadores.items())[:5]:  # Mostrar apenas 5
                            st.code(f"{chave}: Sequencial {valor.get('ultimo_sequencial', 0)}")
                
                # Botão para testar contador
                if st.button("🧪 Testar Contador", key="testar_contador"):
                    teste_numero = contador_persistente.gerar_numero_relatorio(st.session_state.matricula)
                    st.success(f"✅ Número de teste gerado: {teste_numero}")
                    
                    # Mostrar formato
                    ano_t = teste_numero[:4]
                    mat_t = teste_numero[4:8]
                    seq_t = teste_numero[8:]
                    st.info(f"Formato: {ano_t}.{mat_t}.{seq_t}")
                    
                    # Mostrar próximo
                    prox = contador_persistente.obter_proximo_sequencial(st.session_state.matricula)
                    st.info(f"Próximo sequencial será: {prox}")
    ...
    if opcao == "OBRA":
        ...
        if numero_formatado:
            ...
            # Mostrar informações do sistema de contagem
            status_contador = contador_persistente.mostrar_status_contador()
            
            col_info1, col_info2, col_info3 = st.columns(3)
            with col_info1:
                st.metric("🔢 Sequencial", seq)
            with col_info2:
                st.metric("📅 Ano", ano)
            with col_info3:
                st.metric("👤 Matrícula", mat)
            
            # Informações do sistema de persistência
            with st.expander("📊 Sistema de Numeração Persistente", expanded=False):
                st.success("**✅ CONTADOR GARANTIDO - NÃO ZERA APÓS CADA ACESSO**")
                
                st.markdown("""
                **Como funciona:**
                - Cada relatório tem um número único: **ANO(4) + MATRÍCULA(4) + SEQUENCIAL(4)**
                - O sequencial é incrementado e salvo automaticamente
                - Usa **Google Sheets como banco de dados principal**
                - Tem **cache local** como fallback
                - Funciona mesmo no **Streamlit Cloud**
                """)
                
                st.info(f"**Fonte atual:** {status_contador.get('fonte', 'N/A')}")
                st.info(f"**Ambiente:** {status_contador.get('ambiente', 'N/A')}")
                st.info(f"**Última atualização:** {status_contador.get('ultima_atualizacao', 'N/A')}")
                
                # Mostrar histórico recente
                if 'historico_geracao' in st.session_state:
                    historico = st.session_state.historico_geracao[-5:]  # Últimos 5
                    if historico:
                        st.markdown("**Histórico recente:**")
                        for h in reversed(historico):
                            num = h['numero_relatorio']
                            st.caption(f"• {num[:4]}.{num[4:8]}.{num[8:]} - {h['data_hora'][11:19]}")
        ...
        with col_gerar1:
            if st.button("📄 GERAR RELATÓRIO PDF", 
                ...
                # Verificar fonte do contador atual
                status_contador = contador_persistente.mostrar_status_contador()
                if status_contador.get('fonte') == 'arquivo_local_fallback':
                    st.warning("""
                    ⚠️ **Modo Fallback Ativado**
                    
                    O Google Sheets não está disponível no momento.
                    O contador está funcionando em modo local.
                    
                    **Isso significa que:**
                    ✅ O contador NÃO será zerado nesta sessão
                    ⚠️ A persistência entre sessões pode ser limitada
                    💡 Reconecte-se à internet para ativar o modo completo
                    """)
        ...
        with col_gerar2:
            if st.button("🔄 NOVO RELATÓRIO", 
                ...
                # Verificar fonte do contador atual
                status_atual = contador_persistente.mostrar_status_contador()
                ...
                ano_novo = novo_numero[:4]
                mat_novo = novo_numero[4:8]
                seq_novo = novo_numero[8:]
                ...
                # Mensagem de sucesso com informações do contador
                st.success(f"✅ **NOVO RELATÓRIO INICIADO!**")
                st.success(f"🔢 **Número:** {ano_novo}.{mat_novo}.{seq_novo}")
                
                # Mostrar informações do sistema de contagem
                st.info(f"**Fonte do contador:** {status_atual.get('fonte', 'N/A')}")
                
                # Mostrar próximo sequencial
                proximo_apos = contador_persistente.obter_proximo_sequencial(st.session_state.matricula)
                st.info(f"⏭️ **Próximo sequencial será:** {proximo_apos}")
    ...
    # ========== OUTROS MÓDULOS (EM DESENVOLVIMENTO) ==========
        ...
        st.info("📋 Módulo em desenvolvimento Relatório de Fiscalização de empresas.")
        
    elif opcao == "EVENTOS":
        st.title("Relatórios de Eventos")
        st.info("🎯 Módulo em desenvolvimento Relatório de Fiscalização de eventos.")
        
    elif opcao == "AGRONOMIA":
        st.title("Relatórios de Agronomia")
        st.info("📊 Módulo em desenvolvimento Relatório de Fiscalização de Agronomia.")
