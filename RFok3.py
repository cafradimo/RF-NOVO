...
def configurar_pasta_local(matricula, mostrar_logs=False):
    ...
    try:
        ...
        if mostrar_logs:
            st.info(f"🔍 Primeiro acesso: criando pasta para matrícula {matricula}")
        
        # Opções de locais para criar a pasta (em ordem de preferência)
        locais_possiveis = [
            os.path.join(os.path.expanduser("~"), "Documents", nome_pasta),  # 1. Pasta Documents do usuário
            os.path.join(os.path.expanduser("~"), "Desktop", nome_pasta),     # 2. Pasta Desktop
            os.path.join(os.getcwd(), nome_pasta),                           # 3. Pasta no diretório atual
            os.path.join("relatorios", nome_pasta),                          # 4. Pasta relatorios no diretório atual
        ]
        
        if mostrar_logs:
            st.info(f"📍 Tentando criar pasta em: {locais_possiveis[0]}")
        
        # Tenta criar a pasta em cada local possível
        for caminho_pasta in locais_possiveis:
            try:
                ...
                if not os.path.exists(caminho_pasta):
                    ...
                    if mostrar_logs:
                        st.info(f"📁 Pasta já existe em: {caminho_pasta}")
                ...
                if mostrar_logs:
                    st.success(f"🎯 Pasta configurada em: {caminho_pasta}")
                return caminho_pasta
                
            except (PermissionError, OSError) as e:
                if mostrar_logs:
                    st.warning(f"❌ Sem permissão em {caminho_pasta}: {str(e)}")
                continue
            except Exception as e:
                if mostrar_logs:
                    st.warning(f"⚠️ Erro em {caminho_pasta}: {str(e)}")
                continue
