...
# No topo do arquivo, depois das outras importações
...
def carregar_dados_fiscais():
    ...
    try:
        # Verificar se temos configuração do Google Sheets
        if 'google_sheets' not in st.secrets or 'spreadsheet_id' not in st.secrets.google_sheets:
            #st.warning("⚠️ Google Sheets não configurado. Usando fallback local.")
            return carregar_fallback_local()
        
        # Verificar se bibliotecas estão disponíveis
        if not GOOGLE_SHEETS_AVAILABLE:
            st.warning("⚠️ Bibliotecas do Google Sheets não disponíveis. Instale: pip install gspread google-auth")
            return carregar_fallback_local()
        
        # 1. OBTER CREDENCIAIS (do google_drive ou google_sheets)
        creds_info = None
        
        # Primeiro tenta do google_drive (já configurado)
        ...
        # Tenta do google_sheets (se especificado)
        ...
        for i, row in enumerate(dados, 1):
            ...
            if matricula and matricula != 'nan':
                # Formatar matrícula para 4 dígitos
        ...
        if dados_fiscais:
            # st.success(f"✅ {len(dados_fiscais)} fiscais carregados do Google Sheets")
        ...
        # Não mostrar erro na tela principal para não poluir
        # st.error(f"❌ Erro ao carregar do Google Sheets: {str(e)[:200]}")
        return carregar_fallback_local()
        ...
        if os.path.exists(caminho_arquivo):
            ...
            for coluna in colunas_necessarias:
                if coluna not in df.columns:
                    st.warning(f"⚠️ Coluna '{coluna}' não encontrada no arquivo local")
            ...
            if dados_fiscais:
                # st.info(f"📁 {len(dados_fiscais)} fiscais carregados localmente")
                return dados_fiscais
    except Exception as e:
...
class PDF(FPDF):
    ...
    def header(self):
        ...
        if self.logo_data:
            try:
                # Logo em arquivo local (sempre arquivo)
...
def obter_logo():
    """
    Obtém a logo para o PDF e outras partes do sistema
    Sempre retorna o caminho do arquivo "10.png" se existir
    """
    # Lista de possíveis nomes de arquivos de logo (com "10.png" como prioridade)
    logos_possiveis = [
        "10.png",            # Logo específico solicitado
        "logo.png",          # Nome mais comum
        "Logo.png",          # Com L maiúsculo
        "assets/logo.png",   # Pasta assets
        "images/logo.png",   # Pasta images
        "static/logo.png",   # Pasta static
    ]
    
    # Procurar por arquivos de logo localmente
    for logo_path in logos_possiveis:
        if os.path.exists(logo_path):
            return logo_path
    
    # Se não encontrar nenhum logo, criar um placeholder
    try:
        # Criar um logo placeholder simples com PIL
        from PIL import Image, ImageDraw, ImageFont
        img = Image.new('RGB', (200, 100), color=(73, 109, 137))
        d = ImageDraw.Draw(img)
        
        # Tentar usar fonte, mas se não tiver, usar texto simples
        try:
        ...
        # Último recurso: placeholder base64 de uma imagem PNG simples
        placeholder_base64 = ""
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        # Logo na página inicial do login
        try:
    ...
    # ========== MÓDULO OBRA ==========
    if opcao == "OBRA":
        # Cabeçalho com logo e título na página principal
        st.markdown("""
