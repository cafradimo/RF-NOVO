...
try:
    ...
    GOOGLE_SHEETS_AVAILABLE = False
    st.warning("Bibliotecas do Google Sheets não disponíveis. Instale: pip install gspread google-auth")

# ========== FUNÇÃO SIMPLIFICADA: SALVAR NO GOOGLE SHEETS ==========
def salvar_relatorio_google_sheets(dados_relatorio, pdf_path=None):
    """
    Salva os dados do relatório no Google Sheets
    Versão simplificada - usa as mesmas credenciais do Google Drive
    """
    ...
    try:
        ...
        if 'spreadsheet_id' not in st.secrets.google_sheets:
            return False
        ...
        # Obter credenciais do Google Drive (já funcionam)
        ...
        dados_linha.append(dados_relatorio.get('constatacao_fiscal', ''))
        dados_linha.append(dados_relatorio.get('motivo_acao', ''))
        ...
        # Documentos
        dados_linha.append(dados_relatorio.get('documentos_solicitados', ''))
        dados_linha.append(dados_relatorio.get('documentos_recebidos', ''))
        ...
        dados_linha.append(st.session_state.get('drive_link', ''))
...
def salvar_relatorio_google_sheets(dados_relatorio, pdf_path=None):
    ...
    try:
        ...
        # Primeiro tentar obter do google_sheets no secrets
        if 'google_sheets' in st.secrets:
            if 'credentials' in st.secrets.google_sheets:
                ...
                if isinstance(creds_text, str):
                    ...
                    creds_info = creds_text
            elif 'spreadsheet_id' in st.secrets.google_sheets:
                # Tentar credenciais do google_drive como fallback
                if 'google_drive' in st.secrets and 'credentials' in st.secrets.google_drive:
                    creds_text = st.secrets.google_drive.credentials
                    if isinstance(creds_text, str):
                        creds_info = json.loads(creds_text)
        ...
        # Constatação e motivo
        dados_linha.append(dados_relatorio.get('constatacao_fiscal', '')[:100])  # Limitar tamanho
        dados_linha.append(dados_relatorio.get('motivo_acao', '')[:100])  # Limitar tamanho
...
def obter_logo():
    ...
    try:
        ...
        placeholder_base64 = ""
