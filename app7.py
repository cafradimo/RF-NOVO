...
# ========== FUNÇÕES PARA FORMATAÇÃO DE CAMPOS (CORRIGIDAS) ==========
def formatar_latitude_str(valor: str) -> str:
    """
    Formata latitude no padrão -xx.xxxxxx (até 6 casas decimais)
    Exemplo: "23456376" -> "-23.456376"
    Sempre inclui o sinal de menos.
    """
    if not valor:
        return ""
    ...
    if '.' in numeros:
        ...
        inteiro = partes[0][:2]  # máximo 2 dígitos
        ...
        if len(numeros) <= 2:
            ...
            inteiro = numeros[:2]
            decimal = numeros[2:8]  # máximo 6 dígitos decimais
    resultado = inteiro
    if decimal:
        resultado += f".{decimal}"
    # SEMPRE adiciona o sinal de menos
    if resultado:
        resultado = "-" + resultado
    return resultado
...
def formatar_longitude_str(valor: str) -> str:
    """
    Formata longitude no padrão -xx.xxxxxx (até 6 casas decimais)
    Sempre inclui o sinal de menos.
    """
    if not valor:
        return ""
...
class RelatorioPDF(FPDF):
    ...
    def titulo_secao(self, texto):
        """Adiciona um título de seção com fundo azul"""
        fonte_usar = self.fonte_unicode if self.fonte_unicode else 'helvetica'
        self.set_font(fonte_usar, 'B', 11)
        self.set_fill_color(11, 162, 238)  # Azul escuro (Logo)
    ...
    def add_images_to_pdf(self, fotos_info):
        ...
        Título da seção: "06 - FOTOS - Foto X"
        """
        if not fotos_info:
            return
        ...
        for i, foto_info in enumerate(fotos_info, 1):
            ...
            # Título da seção para cada foto (agora 06 - FOTOS)
    ...
    def add_assinatura_agente(self, agente_info):
        """Adiciona a seção de assinatura do agente (08 - ASSINATURA)"""
        fonte_usar = self.fonte_unicode if self.fonte_unicode else 'helvetica'
...
def criar_pdf(dados, logo_path, fotos_info=None, agente_info=None):
    """
    Versão do criar_pdf com as seções:
    01 - ENDEREÇO DO EMPREENDIMENTO
    02 - INTERESSADO
    03 - INTRODUÇÃO
    04 - APURADO
    05 - CONCLUSÃO
    ...
    # Seção 01 - ENDEREÇO DO EMPREENDIMENTO
    ...
    # Seção 06 - FOTOS (cada foto em página separada)
    if fotos_info:
    ...
    # Seção 07 - OUTRAS INFORMAÇÕES (nova página)
    pdf.add_page()
    ...
    # Seção 08 - ASSINATURA (na mesma página da seção 07)
    if agente_info:
...
def limpar_formulario():
    ...
    # Limpa os campos de latitude, longitude e CEP
...
def main():
    ...
    # Campos formatados (único campo por coordenada)
    ...
    # ===== SEÇÃO 07 - OUTRAS INFORMAÇÕES (caixas de texto) =====
    ...
    with col_gerar1:
        if st.button("📄 GERAR RELATÓRIO PDF", type="primary", use_container_width=True,
            ...
            # Verificação dos campos obrigatórios (latitude, longitude, endereço, município)
            ...
            # Prepara o dicionário de dados (usando os valores formatados)
            dados = {
    ...
    with col_gerar2:
        if st.button("🔄 NOVO RELATÓRIO", type="secondary", use_container_width=True,
            ...
            st.session_state.numero_sequencial = 0
            st.session_state.fato_gerador_outro = ""
            # Limpa também os campos formatados
            st.session_state.latitude_input = ""
            st.session_state.longitude_input = ""
            st.session_state.cep_input = ""
