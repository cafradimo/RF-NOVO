...
SCOPES = ['https://www.googleapis.com/auth/drive.file']  # APENAS UM ESCOPO
...
# ========== FUNÇÃO DE AUTENTICAÇÃO CORRIGIDA (BASEADA NO SEU CÓDIGO FUNCIONANDO) ==========
def autenticar_google_drive():
    """Autentica usando OAuth 2.0 com token.json - VERSÃO CORRIGIDA"""
    creds = None
    ...
    if not creds or not creds.valid:
        ...
        if not creds:
            ...
            try:
                ...
                # Para ambiente Streamlit, usamos run_local_server
                creds = flow.run_local_server(
                    port=0,
                    authorization_prompt_message='Por favor, autorize o acesso ao Google Drive',
                    success_message='✅ Autenticação realizada com sucesso! Você já pode fechar esta aba.',
                    open_browser=True
                )
    ...
    try:
        ...
        # Testa a conexão listando arquivos da pasta (com suporte a Drive Compartilhado)
        try:
            ...
            if 'fileNotSupported' in str(e) or 'notFound' in str(e):
                st.sidebar.warning("⚠️ Pasta não encontrada ou sem acesso. Verifique o ID da pasta.")
            else:
...
def upload_para_google_drive(caminho_arquivo, nome_arquivo, service, folder_id=None):
    try:
        ...
        if arquivos:
            ...
            file_metadata = {
                'name': nome_arquivo,
                'description': f'Arquivo do sistema de fiscalização'
            }
            if folder_id:
...
def baixar_arquivo_do_drive(service, nome_arquivo, folder_id):
    """Baixa um arquivo do Google Drive e retorna o caminho temporário"""
    try:
        query = f"name = '{nome_arquivo}' and '{folder_id}' in parents and trashed = false"
        results = service.files().list(
            q=query,
...
# ========== CLASSE CONTADOR DE RELATÓRIOS COM SUPORTE AO GOOGLE DRIVE ==========
class ContadorRelatorios:
    ...
    def carregar_contadores(self):
        ...
        # Primeiro tenta carregar do Google Drive se o serviço estiver disponível
        if self.service:
        ...
        # Se não conseguiu carregar do Drive, cria um contador vazio
        return {}
    ...
    def salvar_contadores(self):
        ...
        try:
            # Cria um arquivo temporário com os contadores
