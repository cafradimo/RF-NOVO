...
CONTADOR_FILE_NAME = "contador_relatorios.json"
...
# ========== FUNÇÃO PARA AUTENTICAR GOOGLE DRIVE NO STREAMLIT CLOUD ==========
@st.cache_resource
def get_drive_service():
    """Inicializa serviço do Google Drive usando service account"""
    try:
        # Para Streamlit Cloud, use secrets
        if 'gdrive' in st.secrets:
            service_account_info = {
                ...
                "private_key": st.secrets["gdrive"]["private_key"],
...
# ========== CLASSE CONTADOR COM PERSISTÊNCIA NO DRIVE ==========
class ContadorRelatorios:
    ...
    def carregar_contadores(self):
        """Carrega contadores do Google Drive"""
        try:
            if not self.drive_service:
                return {}
            
            # Procurar arquivo no Drive
            query = f"name = '{CONTADOR_FILE_NAME}' and '{self.folder_id}' in parents and trashed = false"
            ...
            if arquivos:
                ...
                # Criar novo arquivo com contadores vazios
                contadores_vazios = {}
                self.salvar_contadores(contadores_vazios)
                return contadores_vazios
                
        except Exception as e:
            st.warning(f"⚠️ Erro ao carregar contador: {e}")
            return {}
    
    def salvar_contadores(self, contadores=None):
        """Salva contadores no Google Drive"""
        try:
            if not self.drive_service:
                return False
            ...
            # Converter para JSON
            json_bytes = json.dumps(contadores, indent=2).encode('utf-8')
    ...
    def gerar_numero_relatorio(self, matricula):
        """Gera número único com persistência no Drive"""
        ano = datetime.now().strftime("%Y")
        matricula_formatada = matricula.zfill(4)
        chave = f"{ano}_{matricula_formatada}"
        
        # Recarregar do Drive para garantir consistência
        self.contadores = self.carregar_contadores()
        ...
        # Salvar no Drive
        if self.salvar_contadores():
            st.session_state.contador_atualizado = datetime.now().isoformat()
...
def carregar_dados_fiscais():
    try:
        # No Streamlit Cloud, você pode colocar o arquivo no repositório
...
# ========== FUNÇÕES DO GOOGLE DRIVE PARA PLANILHA ==========
...
def inicializar_planilha_master():
    colunas = [
        ...
        'TIPO_ACAO_OUTROS', 'LATITUDE', 'LONGITUDE', 'ENDERECO', 'NUMERO_ENDERECO', 'COMPLEMENTO',
        'BAIRRO', 'MUNICIPIO', 'UF', 'CEP', 'DESCRITIVO_ENDERECO', 'NOME_CONTRATANTE',
        'REGISTRO_CONTRATANTE', 'CPF_CNPJ_CONTRATANTE', 'CONSTATACAO_FISCAL', 'MOTIVO_ACAO',
        'CARACTERISTICA', 'CARACTERISTICA_OUTROS', 'FASE_ATIVIDADE', 'FASE_ATIVIDADE_OUTROS',
        'NUM_PAVIMENTOS', 'QUANTIFICACAO', 'UNIDADE_MEDIDA', 'UNIDADE_MEDIDA_OUTROS',
        'NATUREZA', 'NATUREZA_OUTROS', 'TIPO_CONSTRUCAO', 'TIPO_CONSTRUCAO_OUTROS',
        'CONTRATADO_01_MESMO_CONTRATANTE', 'CONTRATADO_01_NOME_CONTRATANTE',
        'CONTRATADO_01_REGISTRO_CONTRATANTE', 'CONTRATADO_01_CPF_CNPJ_CONTRATANTE',
        ...
        'CONTRATADO_01_RAMO_ATIVIDADE', 'CONTRATADO_01_SERVICO_EXECUTADO',
        'CONTRATADO_01_SERVICO_OUTROS', 'CONTRATADO_01_FONTE_INFORMACAO',
        'CONTRATADO_01_QUALIFICACAO_FONTE', 'CONTRATADO_01_QUALIFICACAO_OUTROS',
        'TOTAL_CONTRATADOS_REGISTROS', 'DOCUMENTOS_SOLICITADOS', 'DOCUMENTOS_SOLICITADOS_OFICIO_NUMERO',
        ...
        'DOCUMENTOS_SOLICITADOS_DETALHES', 'DOCUMENTOS_RECEBIDOS',
        'DOCUMENTOS_RECEBIDOS_OFICIO_NUMERO', 'DOCUMENTOS_RECEBIDOS_QUADRO_TECNICO',
        'DOCUMENTOS_RECEBIDOS_QUADRO_TECNICO_QUANTIDADE', 'DOCUMENTOS_RECEBIDOS_PRESTADORES',
        'DOCUMENTOS_RECEBIDOS_PRESTADORES_QUANTIDADE', 'DOCUMENTOS_RECEBIDOS_OUTROS',
        'DOCUMENTOS_RECEBIDOS_OUTROS_TEXTO', 'DOCUMENTOS_RECEBIDOS_DETALHES',
        'DATA_RELATORIO_ANTERIOR', 'INFORMACOES_COMPLEMENTARES', 'FONTE_INFORMACAO',
        'QUALIFICACAO_FONTE', 'QUALIFICACAO_FONTE_OUTROS', 'TOTAL_FOTOS', 'FOTOS_COM_COMENTARIOS',
        'AGENTE_NOME', 'AGENTE_MATRICULA', 'AGENTE_UNIDADE', 'DATA_GERACAO',
        'PDF_NOME', 'PDF_LINK', 'PDF_TAMANHO_MB', 'PDF_DATA_UPLOAD'
    ]
...
def preparar_dados_para_planilha_master(dados, agente_info, fotos_info, pdf_info=None,
                                      tipo_visita_outros="", caracteristica_outros="",
                                      fase_atividade_outros="", unidade_medida_outros="",
                                      natureza_outros="", tipo_construcao_outros="",
    ...
    dados_excel = {
        ...
        'PDF_NOME': pdf_info.get('nome', '') if pdf_info else '',
        'PDF_LINK': pdf_info.get('link_visualizacao', '') if pdf_info else '',
        'PDF_TAMANHO_MB': f"{pdf_info.get('tamanho_bytes', 0) / (1024 * 1024):.2f}" if pdf_info else '',
        'PDF_DATA_UPLOAD': pdf_info.get('modificado', pdf_info.get('criado', '')) if pdf_info else ''
    }
...
def adicionar_relatorio_a_planilha_master(dados_relatorio, agente_info, fotos_info, pdf_info, service, folder_id,
    ...
    try:
        ...
        novos_dados = preparar_dados_para_planilha_master(
            dados_relatorio, agente_info, fotos_info, pdf_info,
...
def main():
    ...
    # Inicializar contador com Drive
    contador_manager = ContadorRelatorios(st.session_state.drive_service)
    ...
    with st.sidebar:
        ...
        if hasattr(contador_manager, 'contadores'):
            st.caption(f"📊 Contador sincronizado com Drive")
            ...
            if st.button("📊 Baixar Planilha Master", use_container_width=True, key="download_excel_button"):
                if st.session_state.drive_service:
                    with st.spinner("Carregando Planilha Master..."):
                        df_dados, caminho_temp = carregar_planilha_master_drive(
                            st.session_state.drive_service, GOOGLE_DRIVE_FOLDER_ID
                        )
    ...
    with col_gerar1:
        if st.button("📄 GERAR RELATÓRIO PDF", type="primary", use_container_width=True,
            ...
            if tem_dados_atuais:
                ...
                if current_data_copy.get('servico_executado') == "Outras" and current_data_copy.get('servico_executado_outras'):
                    pass
            ...
            try:
                ...
                with col_drive1:
                    upload_drive = st.checkbox("Enviar para Google Drive", value=True,
                ...
                if upload_drive and st.session_state.drive_service:
                ...
                elif upload_drive and not st.session_state.drive_service:
                    st.warning("⚠️ Serviço do Google Drive não disponível.")
