...
def carregar_credentials():
    """Carrega as credenciais do arquivo ou dos secrets"""
    try:
        # Verifica se já existe o arquivo
        if os.path.exists('credentials.json'):
            return True
        
        # Tenta carregar dos secrets do Streamlit Cloud
        if 'google_service_account' in st.secrets:
            credentials_dict = dict(st.secrets['google_service_account'])
            with open('credentials.json', 'w') as f:
                json.dump(credentials_dict, f)
            st.success("✅ Credenciais carregadas dos secrets!")
            return True
        else:
            st.error("❌ Credenciais não encontradas nem no arquivo nem nos secrets!")
            return False
...
def autenticar_google_drive():
    """Autentica usando service account para acesso a Drive Compartilhado"""
    try:
        # Carrega as credenciais
        if not carregar_credentials():
            return None
        ...
        # Cria o serviço do Drive com suporte a Drive Compartilhado
        service = build('drive', 'v3', credentials=credentials)
        
        # Testa a conexão listando arquivos (importante para shared drives)
        try:
            ...
            st.sidebar.success(f"✅ Drive Conectado: {len(files)} arquivos")
            
        except Exception as e:
            st.sidebar.warning(f"⚠️ Conectado ao Drive, mas pasta pode estar vazia")
        
        return service
        
    except Exception as e:
        st.error(f"❌ Erro ao autenticar com Google Drive: {str(e)}")
        return None
