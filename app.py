...
# ========== FUNÇÃO PARA AUTENTICAÇÃO OAUTH2 (COM TOKEN.JSON) ==========
def autenticar_google_drive():
    """Autentica usando OAuth 2.0 com token.json"""
    creds = None
    token_path = 'token.json'
    credentials_path = 'credentials.json'
    
    # Verifica se existe o token.json
    if os.path.exists(token_path):
        try:
            creds = Credentials.from_authorized_user_file(token_path, SCOPES)
        except Exception as e:
            st.sidebar.warning(f"⚠️ Erro ao ler token.json: {e}")
    
    # Se não há credenciais válidas, tenta atualizar ou criar novas
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                st.sidebar.success("✅ Token atualizado com sucesso!")
            except Exception as e:
                st.sidebar.error(f"❌ Erro ao atualizar token: {e}")
                return None
        else:
            # Tenta criar novas credenciais a partir do arquivo credentials.json
            if os.path.exists(credentials_path):
                try:
                    ...
                    st.sidebar.error(f"❌ Erro na autenticação OAuth: {e}")
                    return None
            else:
    ...
    try:
        ...
        st.sidebar.success(f"✅ Drive Conectado: {len(files)} arquivos encontrados")
        return service
        
    except HttpError as error:
        st.sidebar.error(f'❌ Erro HTTP do Google Drive: {error}')
        return None
    except Exception as e:
        st.sidebar.error(f'❌ Erro ao conectar ao Drive: {str(e)}')
        return None
