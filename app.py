...
def carregar_credentials_dos_secrets():
    """Carrega as credenciais do service account dos secrets do Streamlit Cloud"""
    try:
        import streamlit as st
        if 'google_service_account' in st.secrets:
            # Cria o arquivo credentials.json a partir dos secrets
            credentials_dict = json.loads(st.secrets['google_service_account'])
            with open('credentials.json', 'w') as f:
                json.dump(credentials_dict, f)
            return True
    except Exception as e:
        st.warning(f"⚠️ Erro ao carregar credentials dos secrets: {e}")
    return False
...
CONTADOR_FILENAME = "contador_relatorios.json"  # Nome do arquivo de contadores no Drive
...
def upload_para_google_drive(caminho_arquivo, nome_arquivo, service, folder_id=None):
    try:
        ...
        if arquivos:
            ...
            file_metadata = {
                'name': nome_arquivo,
                'description': f'Arquivo de configuração do sistema'
            }
            if folder_id:
...
def criar_pdf(dados, logo_path, fotos_info=None, agente_info=None):
    ...
    if not contratados_data:
        ...
        for i, contrato in enumerate(contratados_data, 1):
            ...
            # ===== CORREÇÃO: CAMPO "SERVIÇO EXECUTADO:" COM "Outras" =====
            if contrato.get('servico_executado'):
                ...
                # Verifica se o serviço é "Outras" e se há valor personalizado
...
# ========== FUNÇÃO PRINCIPAL ==========
def main():
    # Carregar credentials dos secrets se estiver no Streamlit Cloud
    if not os.path.exists('credentials.json'):
        carregar_credentials_dos_secrets()
    ...
    # ===== SEÇÃO 04 - CORRIGIDA DEFINITIVAMENTE =====
    ...
    with col4:
        ...
        # ===== CORREÇÃO: CAMPO "Outras" DO SERVIÇO EXECUTADO =====
        # Garantir que o campo servico_executado_outras exista no dicionário
        if 'servico_executado_outras' not in current_data:
            current_data['servico_executado_outras'] = ''
        
        if servico_executado == "Outras":
            # Cria o campo de texto com o valor atual
            ...
            # Se o campo não estiver vazio, usa o valor personalizado
            if servico_executado_outras:
                # Mantém o valor original "Outras" no campo principal, mas usamos o personalizado para exibição
                # O campo principal continua "Outras" para referência
                pass
        else:
            # Se não for "Outras", limpa o campo personalizado
            current_data['servico_executado_outras'] = ''
    ...
    if st.button("SALVAR", type="primary", use_container_width=True,
        ...
        # e guarda o valor personalizado no campo separado
        if current_data.get('servico_executado') == "Outras" and current_data.get('servico_executado_outras'):
            # Mantém "Outras" no campo principal para referência
