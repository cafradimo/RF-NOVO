...
# Escopos necessários para drive compartilhado
SCOPES = ['https://www.googleapis.com/auth/drive']  # Escopo completo para drives compartilhados
GOOGLE_DRIVE_FOLDER_ID = "119n021EjT2ilcc7ajUejGLv7mk7Gz8GI"
SHARED_DRIVE_ID = "0AExAXm3UxqZFUk9PVA"  # ID do drive compartilhado
...
# ========== FUNÇÃO DE AUTENTICAÇÃO CORRIGIDA PARA DRIVE COMPARTILHADO ==========
...
def autenticar_service_account():
    ...
    try:
        ...
        # Testa a conexão com o drive compartilhado
        try:
            # Primeiro tenta listar arquivos da pasta específica
            ...
            st.sidebar.success(f"✅ Conectado ao Drive Compartilhado! {len(files)} arquivos encontrados")
            return service
            
        except HttpError as e:
            if 'fileNotSupported' in str(e) or 'notFound' in str(e):
                st.sidebar.warning("⚠️ Pasta não encontrada. Tentando acesso sem drive compartilhado...")
                
                # Tenta sem especificar o drive compartilhado
                results = service.files().list(
                    q=f"'{GOOGLE_DRIVE_FOLDER_ID}' in parents and trashed=false",
                    fields="files(id, name)",
                    supportsAllDrives=True,
                ...
                files = results.get('files', [])
                st.sidebar.success(f"✅ Conectado ao Drive! {len(files)} arquivos encontrados")
                return service
            else:
...
def autenticar_oauth_local():
    ...
    if os.path.exists('token.json'):
        try:
            ...
            st.sidebar.success("✅ Token carregado com sucesso!")
        except Exception as e:
            st.sidebar.warning(f"⚠️ Erro ao carregar token: {e}")
            creds = None
    
    # Se não há credenciais válidas, tenta renovar ou criar novas
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                st.sidebar.warning(f"⚠️ Erro ao renovar token: {e}")
                creds = None
        
        # Se ainda não temos credenciais, inicia fluxo OAuth
        if not creds:
            ...
            try:
                ...
                st.sidebar.success("✅ Nova autenticação realizada com sucesso!")
            except Exception as e:
                st.sidebar.error(f"❌ Erro na autenticação: {str(e)}")
                return None
        
        # Salva o token para próxima execução
        try:
            with open('token.json', 'w') as token:
                token.write(creds.to_json())
            st.sidebar.success("✅ Token salvo com sucesso!")
        except Exception as e:
            st.sidebar.warning(f"⚠️ Não foi possível salvar token: {e}")
    
    # Cria e testa o serviço do Drive
    try:
        service = build('drive', 'v3', credentials=creds)
        
        # Testa a conexão com suporte a drive compartilhado
        results = service.files().list(
            q=f"'{GOOGLE_DRIVE_FOLDER_ID}' in parents and trashed=false",
            fields="files(id, name)",
            supportsAllDrives=True,
        ...
        files = results.get('files', [])
        st.sidebar.success(f"✅ Drive Conectado: {len(files)} arquivos encontrados na pasta")
        return service
...
# ========== FUNÇÕES DO GOOGLE DRIVE CORRIGIDAS ==========
def upload_para_google_drive(caminho_arquivo, nome_arquivo, service, folder_id=None):
    """Upload com suporte a drives compartilhados - CORRIGIDO"""
    try:
        ...
        # Parâmetros comuns para todas as chamadas
        drive_params = {
            'supportsAllDrives': True
        }
        
        # Adiciona driveId se estiver no Streamlit Cloud
        if is_streamlit_cloud() and SHARED_DRIVE_ID:
        ...
        results = service.files().list(
            ...
            supportsAllDrives=True,
            includeItemsFromAllDrives=True,
            corpora='drive' if is_streamlit_cloud() else 'user',
            driveId=SHARED_DRIVE_ID if is_streamlit_cloud() else None
        ).execute()
        ...
        if arquivos:
            # ===== ATUALIZA ARQUIVO EXISTENTE - CORRIGIDO =====
            file_id = arquivos[0]['id']
            
            # Para atualização, NÃO incluímos 'parents' no body
            file = service.files().update(
                fileId=file_id,
                body=file_metadata,  # Sem o campo 'parents' aqui
                media_body=media,
                fields='id, name, webViewLink, webContentLink, size, modifiedTime',
                **drive_params
            ).execute()
            
            # Verifica se precisa mover para a pasta correta
            current_parents = arquivos[0].get('parents', [])
            if folder_id and folder_id not in current_parents:
                # Move o arquivo para a pasta correta se necessário
...
class ContadorRelatorios:
        ...
        if self.service:
            ...
            if caminho_temp:
                try:
                    ...
                    st.warning(f"⚠️ Erro ao carregar contadores do Drive: {e}")
    ...
    def salvar_contadores(self):
        ...
        try:
            ...
            st.error(f"❌ Erro ao salvar contadores no Drive: {e}")
            return False
...
def carregar_planilha_master_drive(service, folder_id):
    try:
        query = f"name = '{EXCEL_DATABASE_NAME}'"
        if folder_id:
            query += f" and '{folder_id}' in parents"
        query += " and trashed = false"
...
def main():
    ...
    if not st.session_state.logged_in:
                    ...
                    if len(matricula_limpa) >= 3 and len(matricula_limpa) <= 4:
                        ...
                        if agente_info:
                            ...
                            if drive_service:
                                ...
                                # Fallback para contador local se o Drive não estiver disponível
        ...
        st.caption("Relatório de Fiscalização - Versão 2.0")
        return
    ...
    if st.button("SALVAR", type="primary", use_container_width=True,
        ...
        # Antes de salvar, garantir que o valor do serviço executado seja armazenado corretamente
        current_data = st.session_state.current_registro.copy()
        
        # Se for "Outras" e tiver valor personalizado, mantém "Outras" no campo principal
    ...
    with col_gerar1:
        if st.button("📄 GERAR RELATÓRIO PDF", type="primary", use_container_width=True,
            ...
            if tem_dados_atuais:
                ...
                # Garantir que o valor personalizado seja usado na criação do dicionário de dados
                current_data_copy = current_registro.copy()
                
                # Se for "Outras" e tiver valor personalizado, preparar para o PDF
                if current_data_copy.get('servico_executado') == "Outras" and current_data_copy.get('servico_executado_outras'):
                    # Mantém "Outras" no campo principal, mas o valor personalizado será usado no PDF
                    pass
            ...
            try:
                ...
                **📊 Resumo Final:**
                ...
                if st.button("📥 Baixar Planilha Master", key=f"download_master_excel_{widget_counter}",
                           use_container_width=True):
                    if drive_service:
                        with st.spinner("Carregando Planilha Master..."):
                            df_dados, caminho_temp = carregar_planilha_master_drive(drive_service, GOOGLE_DRIVE_FOLDER_ID)
    ...
    with col_gerar2:
        if st.button("🔄 NOVO RELATÓRIO", type="secondary", use_container_width=True,
            ...
            # Inicializa o serviço do Drive para gerar novo número
            drive_service = autenticar_google_drive()
            if drive_service:
                contador_manager = ContadorRelatorios(service=drive_service, folder_id=GOOGLE_DRIVE_FOLDER_ID)
            else:
