...
def configurar_pasta_local(matricula):
    """
    ...
    try:
        ...
        # Opções de locais para criar a pasta
        locais_possiveis = [
            os.path.join(os.getcwd(), nome_pasta),  # Pasta no diretório atual
            os.path.join(os.path.expanduser("~"), "Documents", nome_pasta),  # Pasta Documents do usuário
            os.path.join(os.path.expanduser("~"), "Desktop", nome_pasta),  # Pasta Desktop
            os.path.join("relatorios", nome_pasta),  # Pasta relatorios no diretório atual
        ]
            ...
            if arquivo.lower().endswith('.pdf'):
                ...
                try:
                    ...
                    arquivos_pdf.append({
                        ...
                        'numero_relatorio': arquivo.replace('relatorio_', '').replace('.pdf', '')
                    })
                except:
                    continue
...
def abrir_pasta_local(matricula):
    ...
    try:
        ...
        if os.path.exists(pasta_local):
            # Tenta abrir a pasta dependendo do sistema operacional
            import platform
    ...
    if opcao == "OBRA":
            ...
            if st.button("📄 GERAR RELATÓRIO PDF", 
                ...
                try:
                    ...
                    if caminho_salvo:
                        ...
                        # Listar relatórios para mostrar quantos já foram salvos
                        relatorios_salvos = listar_relatorios_local(st.session_state.matricula)
                        if relatorios_salvos:
                            st.info(f"📂 Total de relatórios na sua pasta: {len(relatorios_salvos)}")
        ...
        with col_gerar4:
            if st.button("📂 ABRIR PASTA LOCAL", 
                ...
                sucesso = abrir_pasta_local(st.session_state.matricula)
