...
# ATUALIZADO: Focado apenas no módulo OBRA com armazenamento automático
...
st.set_page_config(
    page_title="Relatório de Fiscalização CREA-RJ - OBRA",
    layout="wide",
    initial_sidebar_state="expanded",
    menu_items={
        'Get Help': 'https://crea-rj.gov.br',
        'Report a bug': None,
        'About': "Sistema de Fiscalização CREA-RJ v2.0 - Módulo OBRA"
    }
)
...
def main():
    ...
    if IS_STREAMLIT_CLOUD:
        with st.expander("⚠️ INFORMAÇÕES IMPORTANTES - STREAMLIT CLOUD", expanded=True):
            ...
            **SISTEMA HOSPEDADO NA NUVEM - ATENÇÃO**
            ...
            - O PDF será salvo automaticamente na pasta RF-CREA-RJ+Matrícula
            - No Streamlit Cloud, a pasta é TEMPORÁRIA
    ...
    if not st.session_state.logged_in:
                ...
                if matricula_input and senha_input:
                    ...
                    if len(matricula_limpa) >= 3 and len(matricula_limpa) <= 4:
                        ...
                        if agente_info:
                            ...
                            # Configurar pasta automaticamente no login
                            ...
                            st.info(f"Pasta criada automaticamente: RF-CREA-RJ{matricula_formatada}")
                            st.rerun()
                        else:
                            agente_info = {
        ...
        st.caption("Versão 2.0 - Relatório de Fiscalização CREA-RJ - Módulo OBRA")
        return
    ...
    with st.sidebar:
        ...
        if info_pasta['existe']:
            st.markdown(f"**Pasta de armazenamento:**")
            st.caption(f"{info_pasta['caminho']}")
            st.caption(f"📄 PDFs salvos: {info_pasta['total_pdfs']}")
            st.caption(f"📊 Ambiente: {info_pasta['ambiente']}")
        
        # Botão para ver relatórios salvos
        if st.button("📋 Ver Relatórios Salvos", use_container_width=True, key="ver_relatorios_salvos"):
    ...
    info_pasta = mostrar_info_pasta(st.session_state.matricula)
    if info_pasta['existe']:
        st.info(f"📁 **Local de armazenamento:** {info_pasta['caminho']}")
        st.caption(f"📊 {info_pasta['ambiente']} - {info_pasta['aviso']}")
        st.caption(f"📄 {info_pasta['total_pdfs']} PDF(s) salvo(s)")
    ...
    constatacao_fiscal = st.text_area("Constatação do Fiscal (Para autuação)", placeholder="Utilizar para possíveis autuações", 
        ...
        if st.button("📄 GERAR RELATÓRIO PDF", 
            ...
            try:
                ...
                if caminho_salvo:
                    ...
                    info_pasta = mostrar_info_pasta(st.session_state.matricula)
                    st.success(f"✅ PDF gerado e salvo em: {info_pasta['caminho']}")
                    ...
                    if relatorios_salvos:
                        ultimo_relatorio = relatorios_salvos[0]
                        st.info(f"📄 **Arquivo:** {ultimo_relatorio['nome']}")
                        st.info(f"📊 **Tamanho:** {ultimo_relatorio['tamanho_kb']:.1f} KB")
                
                # ========== GOOGLE DRIVE UPLOAD ==========
                if GOOGLE_DRIVE_AVAILABLE and GOOGLE_DRIVE_SHARED_DRIVE_ID:
                ...
                **📊 Resumo Final:**
                ...
                if 'drive_resultado' in locals() and drive_resultado:
                    resumo_texto += f"\n- **☁️ Google Drive:** PDF enviado com sucesso!"
                
                if 'sheets_sucesso' in locals() and sheets_sucesso:
