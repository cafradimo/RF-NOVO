...
# MODIFICAÇÃO: Upload obrigatório para Google Drive e link salvo na planilha
...
# ========== NOVA FUNÇÃO: SALVAR NO GOOGLE SHEETS (MODIFICADA) ==========
def salvar_relatorio_google_sheets(dados_relatorio, pdf_path=None):
    ...
    MODIFICAÇÃO: Sempre usa o link do Drive se disponível
    """
    try:
        ...
        # MODIFICAÇÃO: Sempre usar o link do Drive se disponível
        drive_link = st.session_state.get('drive_link', '')
        if not drive_link and 'last_drive_upload' in st.session_state:
            drive_link = st.session_state.last_drive_upload.get('link_visualizacao', '')
        dados_linha.append(drive_link if drive_link else "Arquivo local")
...
def main():
    ...
    if 'last_drive_upload' not in st.session_state:
        st.session_state.last_drive_upload = {}
    ...
    if not st.session_state.logged_in:
        ...
        st.caption("Versão 2.0 - Relatório de Fiscalização CREA-RJ com Download Automático")
        return
        ...
        with col_gerar1:
            if st.button("📄 GERAR RELATÓRIO PDF", 
                ...
                try:
                    ...
                    # ========== GOOGLE DRIVE UPLOAD (OBRIGATÓRIO) ==========
                    ...
                    if drive_service:
                        ...
                        # Usar o arquivo salvo localmente ou temporário
                        arquivo_fonte = caminho_salvo if caminho_salvo else temp_file_path
                        
                        drive_info = upload_para_google_drive(
                            caminho_arquivo=arquivo_fonte,
                        ...
                        if drive_info:
                            ...
                            # SALVAR O LINK NO session_state PARA USO POSTERIOR
                            st.session_state.drive_link = drive_info.get('link_visualizacao', '')
                            st.session_state.last_drive_upload = drive_info  # Backup completo
                            ...
                            <div style="
                                ...
                                text-align: center;
                            ">
                                <h4 style="margin-top: 0;">📁 RELATÓRIO SALVO NA NUVEM!</h4>
                                <p style="font-size: 14px; margin: 8px 0;">
                                    <strong>Nome:</strong> {drive_info['nome']}<br>
                                    <strong>Tamanho:</strong> {tamanho_mb:.2f} MB<br>
                                    <strong>Status:</strong> {drive_info.get('acao', 'ENVIADO')}
                                </p>
                            ...
                            # Mostrar o link também em formato de texto para cópia
                            with st.expander("📋 Copiar Link do Drive", expanded=False):
                                st.code(drive_info['link_visualizacao'], language="text")
                                if st.button("📋 Copiar Link", key="copiar_link_drive"):
                                    st.success("Link copiado para a área de transferência!")
                        else:
                            st.error("❌ Falha ao enviar para o Google Drive. Verifique as credenciais e permissões.")
                            # Continuar mesmo se falhar o Drive
                    else:
                        st.warning("⚠️ Serviço do Google Drive não disponível. O PDF será salvo apenas localmente.")
                    ...
                    **📊 Resumo Final:**
                    ...
                    if 'drive_info' in locals() and drive_info:
                        resumo_texto += f"\n- **☁️ Google Drive:** {drive_info.get('nome', 'PDF enviado')}"
                        resumo_texto += f"\n- **🔗 Link:** Disponível para acesso"
                    elif st.session_state.get('drive_link'):
                        resumo_texto += f"\n- **☁️ Google Drive:** Link disponível na planilha"
                    else:
                        resumo_texto += f"\n- **⚠️ Google Drive:** Não enviado (verifique configurações)"
