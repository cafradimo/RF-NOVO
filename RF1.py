...
# ========== FUNÇÃO DETECTAR AMBIENTE ==========
def detectar_ambiente():
    """Detecta se estamos no Streamlit Cloud ou ambiente local"""
    # Verificar variável de ambiente do Streamlit Cloud
...
# ========== FUNÇÃO PARA CRIAR/CONFIGURAR PASTA LOCAL (OTIMIZADA) ==========
def configurar_pasta_local(matricula, mostrar_logs=False):
    ...
    try:
        ...
        if ambiente == "streamlit_cloud":
            ...
            # No Streamlit Cloud, o usuário é 'appuser' e não temos permissão em /home
            # Vamos criar na pasta de trabalho atual do app
            caminho_atual = os.getcwd()
            
            # Tentar criar na pasta 'relatorios' dentro do diretório do app
            caminho_pasta = os.path.join(caminho_atual, "relatorios", nome_pasta)
            
            # Garantir que o diretório relatorios existe
            ...
            locais_possiveis = [
                ...
                os.path.join(os.getcwd(), "relatorios", nome_pasta),              # 3. Pasta relatorios no diretório atual
        ...
        # ==============================================
        # CRIAÇÃO FINAL DA PASTA (fallback)
        # ==============================================
        try:
...
def interface_escolher_pasta(matricula):
    ...
    if ambiente == "streamlit_cloud":
        st.warning("⚠️ **Você está no Streamlit Cloud**")
        st.info("""
        No Streamlit Cloud, os arquivos são armazenados dentro do diretório do aplicativo.
        Recomendamos usar a pasta 'relatorios' automática.
        
        **Limitações do Streamlit Cloud:**
        - Não é possível acessar diretamente seu sistema de arquivos local
        - Os arquivos ficam dentro do contêiner da aplicação
        - Para backup, use o download direto dos PDFs ou o Google Drive
        """)
        
        # Opções específicas para Streamlit Cloud
        opcoes = {
            "📁 Pasta Relatórios (Automático)": "relatorios",
            "📂 Diretório do Sistema": "atual"
        }
    ...
    if escolha == "🔧 Personalizado (escolher manualmente)":
        if ambiente == "streamlit_cloud":
            st.warning("No Streamlit Cloud, apenas o diretório do aplicativo é acessível.")
            st.info("Use uma das opções acima.")
            return None
        ...
        # Pasta relatorios no diretório atual
    ...
    # Adicionar informação sobre ambiente
    if ambiente == "streamlit_cloud":
        st.info("📍 **No Streamlit Cloud:** Esta pasta ficará dentro do diretório do aplicativo.")
...
def abrir_pasta_local(matricula):
    ...
    try:
        ...
        if ambiente == "streamlit_cloud":
            # No Streamlit Cloud, não podemos abrir explorador de arquivos
            pasta_local = configurar_pasta_local(matricula)
