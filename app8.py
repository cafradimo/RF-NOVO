...
st.set_page_config(
    page_title="Relatório de Inteligência Fiscal",
    layout="wide",
...
# ========== ORDEM DAS COLUNAS (para garantir posição da DEMANDA) ==========
COLUNAS_ORDEM = [
    'NUMERO_RELATORIO',
    'SITUAÇÃO',
    'DEMANDA',                # <--- Terceira coluna
    'DATA_RELATORIO',
    'FATO_GERADOR',
    'PROTOCOLO',
    'TIPO_ACAO',
...
def formatar_latitude_str(valor: str) -> str:
    if not valor:
        return ""
    ...
    if '.' in numeros:
        ...
        inteiro = partes[0][:2]
        decimal = partes[1][:6] if len(partes) > 1 else ""
    else:
        if len(numeros) <= 2:
            inteiro = numeros
            decimal = ""
...
def formatar_longitude_str(valor: str) -> str:
    if not valor:
        return ""
    numeros = re.sub(r'[^\d\.]', '', valor)
    if not numeros:
...
def formatar_cep_str(valor: str) -> str:
    if not valor:
        return ""
...
def atualizar_latitude():
    raw = st.session_state.get("latitude_input", "")
    st.session_state.latitude_input = formatar_latitude_str(raw)

def atualizar_longitude():
    raw = st.session_state.get("longitude_input", "")
    st.session_state.longitude_input = formatar_longitude_str(raw)

def atualizar_cep():
    raw = st.session_state.get("cep_input", "")
    st.session_state.cep_input = formatar_cep_str(raw)
...
# ========== FUNÇÃO PARA GERENCIAR PASTA LOCAL ==========
def get_pasta_local(matricula):
    if is_streamlit_cloud():
        temp_dir = tempfile.gettempdir()
        nome_pasta = f"RIF-CREA-RJ-{matricula}"
...
def disponibilizar_pdf_para_download(caminho_arquivo, nome_arquivo):
    try:
        ...
        if is_streamlit_cloud():
            ...
            3. Crie a pasta **RIF-CREA-RJ-MATRICULA** se necessário
...
# ========== FUNÇÃO PARA CARREGAR SENHAS DO GOOGLE DRIVE ==========
...
def reordenar_colunas(df):
    """Reordena as colunas do DataFrame conforme COLUNAS_ORDEM, mantendo as que existem."""
    colunas_existentes = [col for col in COLUNAS_ORDEM if col in df.columns]
    # Adiciona colunas que existem no df mas não estão na ordem definida (caso haja)
    colunas_extra = [col for col in df.columns if col not in COLUNAS_ORDEM]
    colunas_final = colunas_existentes + colunas_extra
...
def adicionar_relatorio_a_planilha_master(dados_relatorio, agente_info, fotos_info, service, folder_id,
    ...
    try:
        ...
        # Reordena as colunas antes de salvar
        df_existente = reordenar_colunas(df_existente)
...
def preparar_dados_para_planilha_master(dados, agente_info, fotos_info, qualificacao_outros=""):
    dados_excel = {
        ...
        'DEMANDA': dados.get('demanda', ''),   # <-- DEMANDA agora na terceira posição
...
class RelatorioPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(190, 10, 'RELATÓRIO DE INTELIGÊNCIA FISCAL', 0, 1, 'C')
        if self.agente_info:
    ...
    def titulo_secao(self, texto):
        ...
        self.set_fill_color(11, 162, 238)
    ...
    def add_images_to_pdf(self, fotos_info):
        ...
        margem_superior = 50
        margem_inferior = 30
        area_disponivel = altura_total_pagina - margem_superior - margem_inferior
        ...
        for i, foto_info in enumerate(fotos_info, 1):
            self.add_page()
            self.titulo_secao(f"06 - FOTOS - Foto {i}")
            self.set_font(fonte_usar, 'B', 10)
    ...
    def add_assinatura_agente(self, agente_info):
        fonte_usar = self.fonte_unicode if self.fonte_unicode else 'helvetica'
        self.titulo_secao("08 - ASSINATURA")
        self.ln(4)
        self.ln(12)
        self.set_font(fonte_usar, '', 10)
        x_center = (210 - 120) / 2
...
# ========== FUNÇÃO CRIAR PDF (REESTRUTURADA) ==========
def criar_pdf(dados, logo_path, fotos_info=None, agente_info=None):
    ...
    if dados.get('demanda'):
        pdf.campo("Demanda", dados.get('demanda', ''))
    ...
    pdf.titulo_secao("01 - LOCALIZAÇÃO DO PROTOCOLO/PROCESSO")
...
def limpar_formulario():
    ...
    if 'latitude_input' in st.session_state:
        st.session_state.latitude_input = ""
    if 'longitude_input' in st.session_state:
...
def main():
    ...
    if not st.session_state.logged_in:
        st.title("Relatório de Inteligência Fiscal")
        ...
        st.markdown("Leonardo Bride - 2026")
        st.caption("Relatório de Inteligência Fiscal - Versão 1.0")
        return
    ...
    st.markdown("Preencha os dados abaixo para gerar o relatório de inteligência fiscal.")
    ...
    with col1:
        ...
        demanda = st.selectbox("Demanda", ["", "Denúncia Identificada", "Denúncia Anônima", "Defesa de Auto de Infração", "Ministério Público", "Certidão de Acervo Técnico"], 
                               key=f"demanda_select_{widget_counter}")
    with col2:
        data_relatorio = st.date_input("Data do Relatório", value=datetime.now(), 
                                      format="DD/MM/YYYY",
                                      key=f"data_relatorio_input_{widget_counter}")
        opcoes_fato_gerador = ["", "Processo/Protocolo", "Outro"]
    ...
    # ===== SEÇÃO 01 =====
    st.markdown("### 01 - LOCALIZAÇÃO DO PROTOCOLO/PROCESSO")
    st.subheader("Coordenadas do Local")
    col_lat, col_lon = st.columns(2)
    with col_lat:
        st.text_input(
            "Latitude *", 
            placeholder="Ex: -23.456376", 
            key="latitude_input",
            on_change=atualizar_latitude
        )
        st.caption("Formato: -xx.xxxxxx (até 6 casas decimais)")
    with col_lon:
        st.text_input(
            "Longitude *", 
            placeholder="Ex: -43.633308", 
            key="longitude_input",
            on_change=atualizar_longitude
        )
        st.caption("Formato: -xx.xxxxxx (até 6 casas decimais)")
    ...
    with col1:
        st.text_input(
            ...
            key="cep_input",
            on_change=atualizar_cep
        )
        st.caption("Formato: 00.000-000")
    with col2:
    ...
    # ===== SEÇÃO 03 =====
    st.markdown("### 03 - INTRODUÇÃO")
    apurado_introducao = st.text_area("Introdução:", 
                                      placeholder="Contextualização inicial do que motivou a fiscalização, internamente",
                                      key=f"apurado_introducao_textarea_{widget_counter}",
                                      height=100)

    # ===== SEÇÃO 04 =====
    st.markdown("### 04 - APURADO")
    apurado_apurado = st.text_area("O que foi apurado:", 
                                   placeholder="Descrição detalhada do que foi verificado e constatado durante a fiscalização, internamente",
    ...
    apurado_conclusao = st.text_area("Conclusão:", 
                                     placeholder="Conclusão final sobre a fiscalização realizada, constatado internamente",
                                     key=f"apurado_conclusao_textarea_{widget_counter}",
                                     height=100)

    # ===== SEÇÃO 06 - FOTOS =====
    st.markdown("### 06 - FOTOS - REGISTRO FOTOGRÁFICO")
    if 'temp_photo_bytes' not in st.session_state:
        st.session_state.temp_photo_bytes = None
    ...
    with col_gerar1:
        if st.button("📄 GERAR RELATÓRIO PDF", type="primary", use_container_width=True,
            ...
            latitude_valor = st.session_state.latitude_input
            longitude_valor = st.session_state.longitude_input
            cep_valor = st.session_state.cep_input
            if not latitude_valor or not longitude_valor:
            ...
            total_fotos = len(st.session_state.fotos_info)
            if total_fotos == 0:
                st.info("ℹ️ Nenhuma foto adicionada. O relatório será gerado sem fotos.")
            ...
            dados = {
                ...
                'protocolo': protocolo,
                'latitude': latitude_valor,
                'longitude': longitude_valor,
                'endereco': endereco,
                'numero': numero,
