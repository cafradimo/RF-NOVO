...
class ContadorRelatorios:
    ...
    def reservar_numero(self, matricula):
        """
        Reserva um número para esta sessão
        Retorna o número reservado e salva no .json para bloquear
        """
        ano = datetime.now().strftime("%Y")
        matricula_formatada = matricula.zfill(4)
        chave = f"{ano}_{matricula_formatada}"
        
        # Se já existe no .json, pega o próximo
        if chave in self.contadores:
            proximo_numero = self.contadores[chave] + 1
        else:
            proximo_numero = 1
        
        # JÁ RESERVA no .json (bloqueia o número)
        self.contadores[chave] = proximo_numero
        
        # Salva no Drive para outros usuários verem o bloqueio
        if self.service:
    ...
    def confirmar_numero(self, matricula, numero_sequencial):
        """
        Confirma que o número foi usado (já está reservado, não precisa incrementar)
        Este método é chamado quando o relatório é finalizado
        """
        # O número já está reservado, não precisa fazer nada
        # Mas podemos registrar que foi usado (opcional)
        pass
    
    def liberar_numero(self, matricula, numero_sequencial):
        """
        Libera o número caso o relatório seja cancelado
        Volta o contador para o valor anterior
        """
        ano = datetime.now().strftime("%Y")
        matricula_formatada = matricula.zfill(4)
        chave = f"{ano}_{matricula_formatada}"
        
        if chave in self.contadores and self.contadores[chave] == numero_sequencial:
            # Se o último número reservado é este, volta para o anterior
            self.contadores[chave] = max(1, numero_sequencial - 1)
...
def main():
    ...
    if 'numero_sequencial' not in st.session_state:  # NOVO: guarda apenas o número sequencial
    ...
    if 'contador_manager' not in st.session_state:  # NOVO: guarda o manager na sessão
                ...
                if matricula_input:
                    ...
                    if len(matricula_limpa) >= 3 and len(matricula_limpa) <= 4:
                        ...
                        if agente_info:
                            ...
                            # RESERVA um número (já bloqueia no .json)
                            numero_completo, numero_seq = contador_manager.reservar_numero(matricula_formatada)
                            ...
                            st.info(f"Número do relatório gerado (reservado): {numero_completo}")
                            st.rerun()
                        else:
    ...
    with st.sidebar:
        ...
        if st.button("Sair", type="secondary", use_container_width=True, key="logout_button"):
            # Se sair sem finalizar, libera o número reservado (opcional)
            if st.session_state.contador_manager and st.session_state.numero_sequencial > 0:
                st.session_state.contador_manager.liberar_numero(
                    st.session_state.matricula, 
                    st.session_state.numero_sequencial
                )
    ...
    with col_gerar1:
        if st.button("📄 GERAR RELATÓRIO PDF", type="primary", use_container_width=True,
            ...
            try:
                ...
                # ===== USAR FUNÇÃO ADAPTADA PARA SALVAR PDF =====
                ...
                if caminho_pdf:
                    ...
                    if drive_service:
                        ...
                        if excel_sucesso:
                            ...
                            # Confirma que o número foi usado (opcional)
                            if st.session_state.contador_manager:
                                st.session_state.contador_manager.confirmar_numero(
                                    st.session_state.matricula,
                                    st.session_state.numero_sequencial
                                )
                        else:
    ...
    with col_gerar2:
        if st.button("🔄 NOVO RELATÓRIO", type="secondary", use_container_width=True,
            ...
            # Usa o manager guardado na sessão para reservar novo número
            if st.session_state.contador_manager:
                numero_completo, numero_seq = st.session_state.contador_manager.reservar_numero(
                    st.session_state.matricula
                )
                st.session_state.numero_relatorio_gerado = numero_completo
                st.session_state.numero_sequencial = numero_seq
                ...
                numero_completo, numero_seq = contador_manager.reservar_numero(st.session_state.matricula)
                st.session_state.numero_relatorio_gerado = numero_completo
                st.session_state.numero_sequencial = numero_seq
                st.session_state.contador_manager = contador_manager
            ...
            st.session_state.secao04_limpa_counter = 0
            st.session_state.form_widget_counter += 1
            st.success(f"✅ Novo relatório iniciado: {numero_completo}")
