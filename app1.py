...
def get_pasta_local(matricula, caminho_base=None):
    """
    Retorna o caminho da pasta local para salvar os PDFs
    Se caminho_base for None, usa a pasta Documentos do usuário
    """
    if caminho_base is None:
        # Tenta encontrar a pasta Documentos
        home = str(Path.home())
        possiveis_caminhos = [
            os.path.join(home, 'Documents'),
            os.path.join(home, 'Documentos'),
            os.path.join(home, 'Meus Documentos'),
            home  # Último recurso: usar o diretório home
        ]
        ...
        if caminho_base is None:
            caminho_base = home  # Fallback para home directory
    ...
    if not os.path.exists(caminho_pasta):
        ...
        st.success(f"📁 Pasta criada em: {caminho_pasta}")
...
# ========== CLASSE CONTADOR DE RELATÓRIOS (NA NUVEM) ==========
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        st.caption("Relatório de Fiscalização - Versão 2.0 (PDF Local + Planilha na Nuvem)")
        return
    ...
    with col_gerar1:
        if st.button("📄 GERAR RELATÓRIO PDF", type="primary", use_container_width=True,
            ...
            try:
                ...
                # ===== SALVAR PDF NA PASTA LOCAL =====
                status_text.text("📁 Copiando PDF para pasta local...")
                nome_arquivo_pdf = f"relatorio_{st.session_state.numero_relatorio_gerado}.pdf"
                caminho_pdf_local = os.path.join(st.session_state.pasta_local, nome_arquivo_pdf)
                
                # Copia o arquivo para a pasta local
                import shutil
                shutil.copy2(temp_file_path, caminho_pdf_local)
                
                st.success(f"✅ PDF salvo em: {caminho_pdf_local}")
                
                # ===== ATUALIZAR PLANILHA MASTER NA NUVEM =====
                ...
                if drive_service:
                    ...
                    st.warning("⚠️ Não foi possível conectar ao Google Drive para atualizar a Planilha Master. Os dados serão salvos apenas localmente no PDF.")
                ...
                # Mostra o caminho onde o PDF foi salvo
                st.info(f"📁 **PDF também salvo em:** `{caminho_pdf_local}`")
                ...
                **📊 Resumo Final:**
                ...
                - **Local do arquivo:** {caminho_pdf_local}
                """
                
                if excel_sucesso:
                    resumo_texto += "\n- **📊 Planilha Master:** Dados atualizados com sucesso na nuvem!"
                else:
                    resumo_texto += "\n- **⚠️ Planilha Master:** Falha na atualização (apenas PDF local)"
